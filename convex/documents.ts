...
export const getDocuments = query({
  ...
  handler: async (ctx,{search,paginationOpts}) => {
    ...
    if(search && orgId){
      return await ctx.db.query("documents").withSearchIndex("search_title",(q)=>q.search("title",search).eq("organizationId",orgId))
...
export const deleteById = mutation({
  ...
  handler:async(ctx,args)=>{
    ...
    const isAdmin =  user.organization_role?.toString().split(":")[1] == 'admin'
    if(!isOwner && !isAdmin){
      throw new ConvexError("Unauthorized")
